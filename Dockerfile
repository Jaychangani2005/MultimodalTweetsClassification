...
# # Command to run Jupyter Notebook in the notebooks folder
# CMD ["jupyter", "notebook", "--ip=0.0.0.0", "--port=8888", "--no-browser", "--allow-root", "--notebook-dir=/app/MultimodalTweetsClassification/notebooks"]
...
# Install build tools, dependencies, and Jupyter
RUN apt-get update && \
    apt-get install -y build-essential && \
    pip install --no-cache-dir jupyter -r requirement.txt && \
    apt-get purge -y --auto-remove build-essential && \
